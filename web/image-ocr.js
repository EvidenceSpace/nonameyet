...
function base(file) {
  return { fileId: file.id, caseId: file.caseId, fileHash: file.sha256, updatedAt: new Date().toISOString() };
}
...
function bounded(operation, signal, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(reject, codedError("Local OCR timed out.", "ocr_timeout")), timeoutMs);
...
export async function processImage(file, { signal, runtime = browserTextDetectorRuntime(), timeoutMs = IMAGE_OCR_TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    const result = await bounded(runtime.recognize({ bytes, mimeType: file.type, signal }), signal, timeoutMs);
    if (signal?.aborted) throw new DOMException("cancelled", "AbortError");
    ...
    return {
      ...
      artifact: { adapterId: "local-image-ocr", adapterVersion: `1.0.0+${runtime.id}-${runtime.version}`, pages: [{ pageNumber: 1, text, start: 0, end: text.length }], text, warnings, quality },
    };
  } catch (error) {
