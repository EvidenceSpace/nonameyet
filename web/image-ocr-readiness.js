...
export function inspectImageOcrReadiness(scope = globalThis) {
  ...
  if (hasTextDetector && hasImageBitmap) return { available: true, state: "ready", label: "Local OCR ready", message: "On-device image text detection is available. Original image bytes stay in this browser." };
  return { available: false, state: "unavailable", label: "Local OCR unavailable", message: IMAGE_OCR_UNAVAILABLE_MESSAGE, missing: [!hasTextDetector && "text_detector", !hasImageBitmap && "image_decoder"].filter(Boolean) };
}

const PERMANENT_IMAGE_FAILURE_CODES = new Set(["unsupported_type", "corrupt_file", "file_too_large", "empty_text", "output_too_large", "original_hash_mismatch", "invalid_original_hash"]);
