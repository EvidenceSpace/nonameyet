...
const PDF_HASH = createHash("sha256").update(new Uint8Array(pdfBytes())).digest("hex");
function file(arrayBuffer = async () => pdfBytes()) {
  return { id: "pdf-1", caseId: "case-1", sha256: PDF_HASH, original: { arrayBuffer } };
}
...
test("successful extraction still releases resources and preserves provenance", async () => {
  ...
  assert.equal(result.fileHash, PDF_HASH);
