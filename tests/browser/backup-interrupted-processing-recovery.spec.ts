...
test("restored in-progress extraction becomes an explicit retryable failure", async ({ page }) => {
  ...
  await page.evaluate(async (message) => {
    ...
    const [recovered] = backup.recoverInterruptedProcessing([{
      fileId: file.id,
      caseId,
      fileHash: file.sha256,
      status: "extracting",
      message: "Running OCR locally in this browser…",
      updatedAt: "2026-08-02T23:00:00.000Z",
      artifact: { text: "partial output that must be discarded" },
      nextRetryAt: "2026-08-02T23:01:00.000Z",
