...
test("marks empty local OCR output as permanent without an artifact", async ({ page }) => {
  ...
  await seedWorkspaceFiles(page, [{ id: "file_no_readable_text", name: "unreadable-message.png", mimeType: "image/png", buffer: imageBytes }]);
  ...
  await page.reload(); await waitForWorkspace(page); const persisted = page.locator(".file-row", { hasText: "unreadable-message.png" }); await expect(persisted.locator(".processing-status")).toHaveText("Needs attention"); await expect(persisted.locator(".process-file")).toHaveCount(0);
});
