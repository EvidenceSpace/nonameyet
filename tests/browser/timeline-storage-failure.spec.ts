...
test("timeline create, edit, and remove failures preserve records and drafts", async ({ page }) => {
  ...
  await page.goto("/cases-new.html"); await page.locator("#case-title").fill("Timeline rollback"); await page.locator("#client").fill("Example client"); await page.locator("#amount").fill("5000"); await page.locator("#summary").fill("A synthetic case verifying source-linked timeline actions remain trustworthy when local storage fails."); await page.locator("#continue-button").click(); await page.locator("#continue-button").click(); await page.locator("#local-storage-ack").check(); await page.locator("#continue-button").click(); await page.locator("#open-workspace").click(); await expect(page.locator("#workspace")).toBeVisible(); await page.locator("#file-input").setInputFiles({ name: "delivery.png", mimeType: "image/png", buffer: imageBytes });
