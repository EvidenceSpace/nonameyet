...
test("timeline create, edit, and remove failures preserve records and drafts", async ({ page }) => {
  ...
  await page.locator("#case-title").fill("Timeline rollback");
  await page.locator("#client").fill("Example client");
  await page.locator("#amount").fill("5000");
  await page.locator("#summary").fill("A synthetic case verifying source-linked timeline actions remain trustworthy when local storage fails.");
  await page.locator("#continue-button").click();
  await page.locator("#continue-button").click();
  await page.locator("#local-storage-ack").check();
  await page.locator("#continue-button").click();
  await page.locator("#open-workspace").click();
  await seedWorkspaceFiles(page, [{ id: "file_timeline_rollback", name: "delivery.png", mimeType: "image/png", buffer: imageBytes }]);
  ...
  expect(await page.evaluate(async (id) => (await (await import("/storage.js")).getEventsForCase(id)).length, caseId)).toBe(0);
  expect(await page.evaluate(async (id) => (await (await import("/storage.js")).getCase(id)).updatedAt, caseId)).toBe(originalUpdatedAt);
  await dialog.locator(".timeline-cancel").click();
  const seeded = await page.evaluate(async (id) => {
    const s = await import("/storage.js");
    const [file] = await s.getFilesForCase(id);
    const event = { id: "event_timeline_rollback", caseId: id, eventDate: "2026-08-03", title: "Final files delivered", description: "Delivery noted in the selected screenshot.", sourceFileId: file.id, sourceReference: { fileId: file.id, sha256: file.sha256, locator: { kind: "whole_file", quote: "Final files attached." } }, origin: "user_entered", revision: 1, createdAt: "2026-08-03T00:00:00.000Z", updatedAt: "2026-08-03T00:00:00.000Z" };
    await s.saveEvent(event);
    return { event, updatedAt: (await s.getCase(id)).updatedAt };
  }, caseId);
  await page.reload();
  await waitForWorkspace(page);
  const card = page.locator('.timeline-event[data-id="event_timeline_rollback"]');
  ...
  const afterEdit = await page.evaluate(async (id) => ({ event: (await (await import("/storage.js")).getEventsForCase(id))[0], updatedAt: (await (await import("/storage.js")).getCase(id)).updatedAt }), caseId);
  expect(afterEdit.event.title).toBe(seeded.event.title);
  expect(afterEdit.event.revision).toBe(1);
  expect(afterEdit.updatedAt).toBe(seeded.updatedAt);
  await dialog.locator(".timeline-cancel").click();
  ...
  expect(await page.evaluate(async (id) => (await (await import("/storage.js")).getEventsForCase(id)).length, caseId)).toBe(1);
  expect(await page.evaluate(async (id) => (await (await import("/storage.js")).getCase(id)).updatedAt, caseId)).toBe(seeded.updatedAt);
  await page.reload();
  await waitForWorkspace(page);
  await expect(page.locator('.timeline-event[data-id="event_timeline_rollback"]')).toBeVisible();
